...
const { enviarTicketCompra, enviarCorreoVendedor } = require('./emailController');
...
const orderController = {
  async crearPedido(req, res) {
    ...
    try {
      ...
      // ── 11. Obtener tienda del vendedor ──────────────────────────────────
      const { data: productoConTienda } = await supabase
        .from('productos')
        .select('tienda_id, tiendas(email, nombre_tienda, user_id)')
        .eq('id', itemsCarrito[0].producto_id)
        .single();

      const tienda   = productoConTienda?.tiendas;
      const tiendaId = productoConTienda?.tienda_id;

      // ── 12. Crear notificación para el vendedor ──────────────────────────
      if (tiendaId) {
        const itemsResumen = detallesData.map(item => {
          const prod = itemsCarrito.find(i => i.producto_id === item.producto_id);
          return `${item.cantidad}x ${prod?.productos?.nombre_producto ?? 'Producto'}`;
        }).join(', ');
      ...
      // ── 13. Enviar correo al vendedor ────────────────────────────────────
      if (tienda?.email) {
        await enviarCorreoVendedor({
          tienda,
      ...
      // ── 14. Registrar pago en tabla pagos ────────────────────────────────
      if (pago) {
