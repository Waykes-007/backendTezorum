...
const enviarTicketCompra = async ({ pedido, cliente, items }) => {
  try {
    // ── 1. Generar QR y subirlo a Supabase Storage ──
    ...
    // ── 2. Fecha prometida (3 días hábiles) ──
    const fechaEntrega = new Date();
    fechaEntrega.setDate(fechaEntrega.getDate() + 3);
    const diasSemana = ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado'];
    const meses = ['enero', 'febrero', 'marzo', 'abril', 'mayo', 'junio', 'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre'];
    const fechaTexto = `el ${diasSemana[fechaEntrega.getDay()]} ${fechaEntrega.getDate()} de ${meses[fechaEntrega.getMonth()]}`;
    ...
    // ── 4. HTML del ticket estilo el modelo ──
    const html = `
    <!DOCTYPE html>
    <html>
    <head>
